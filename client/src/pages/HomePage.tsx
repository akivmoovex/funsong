...
export function HomePage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <section>
        <Card surface="lobby" className="space-y-4 p-4 sm:p-5" data-testid="home-join-party-card">
          <h2 className="text-xl font-black text-white sm:text-2xl">Join Party</h2>
          <p className="text-sm text-white/85">
            <span className="font-bold text-white/95">Two ways to join:</span> ask the host for a{' '}
            <span className="font-bold text-amber-100">scan of their QR</span> (opens the app at the
            right room), or <span className="font-bold text-cyan-100">enter the party code</span> below
            to open the same join page.
          </p>
          <ManualPartyCodeJoinForm idPrefix="home" className="space-y-0" />
        </Card>
      </section>
    </div>
  )
